...
# username/password/PIN, then just redeploy - no separate script or
...
OWNER_PIN = "83728957"  # unlock PIN shown after leaving/returning to the site


def _ensure_owner_account():
    """Runs on every app startup. Guarantees OWNER_USERNAME/OWNER_PASSWORD/
    OWNER_PIN always work for an "owner" account, no matter what's in the
    database - creates the account if it's missing, or force-resets its
    password/PIN/role if it already exists with something else. This is
