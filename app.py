...
def create_app():
    ...
    # ---- TEMPORARY: login/password check fully disabled ----
    # Every request is auto-logged-in as the owner account below, so no
    # password is needed anywhere in the app right now. This is meant to
    # be temporary - see the "TO RE-ENABLE THE LOGIN SCREEN" note further
    # down in this file for how to turn the password check back on later.
    @app.before_request
    def _auto_login():
...
# Hardcoded owner login - change these two values any time you want a
# different username/password, then just redeploy (no separate script
# or Render dashboard steps needed).
OWNER_USERNAME = "Rimon1340"
OWNER_PASSWORD = "629791"
...
# ---- TO RE-ENABLE THE LOGIN SCREEN LATER ----
# When you're ready to turn the password check back on:
# 1. Delete the whole "@app.before_request def _auto_login(): ..." block
#    above (the one under "TEMPORARY: login/password check fully disabled").
# 2. Redeploy.
# That's it - /login will immediately start asking for a password again,
# and Rimon1340 / 629791 (set near the top of this file) will still work.
