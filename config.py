...
class Config:
    ...
    # ---- Login session ----
    # How long a login stays valid with NO activity at all. Every request
    # slides this forward (see app.py's before_request), so someone
    # actively using the site never gets logged out mid-task - this only
    # kicks in once they've been away (app minimized, phone locked, browser
    # closed, etc.) for longer than this many minutes.
    SESSION_TIMEOUT_MINUTES = int(os.environ.get("SESSION_TIMEOUT_MINUTES", "20"))
