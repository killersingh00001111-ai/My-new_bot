...
# The three client platforms an admin can connect to this payment system
# through the external API (see external_api.py) - each gets its own
# separate API ID + Callback ID, generated once per admin and kept for as
# long as that admin's account exists (see get_or_create_api_credentials
# below).
API_PLATFORMS = ("android", "website", "telegram")
API_PLATFORM_LABELS = {
    "android": "Android APK",
    "website": "Website",
...
class User(UserMixin, db.Model):
    ...
    def display_expires_at(self):
        """What to show in the "Expires" column - for the owner to see,
        right away, when a newly created admin's password will run out.
        Uses the real expires_at once it exists (set at first login - see
        auth.py's login()). Until then, shows a projection counted from
        account-creation time instead, purely so there's something to
        look at immediately. This is DISPLAY ONLY - is_expired/can_login
        below still only ever look at the real expires_at, so an admin
        who hasn't logged in yet still can't actually be locked out
        early, no matter what this shows."""
        if self.expires_at:
            return self.expires_at
        if self.duration_seconds and self.created_at:
...
class ApiCredential(db.Model):
    ...
    platform = db.Column(db.String(20), nullable=False)  # one of API_PLATFORMS
    api_id = db.Column(db.String(64), unique=True, nullable=False)
    callback_id = db.Column(db.String(64), unique=True, nullable=False)
    created_at = db.Column(db.DateTime, default=datetime.utcnow)

    user = db.relationship("User", backref=db.backref("api_credentials", lazy="dynamic"))

    __table_args__ = (
        db.UniqueConstraint("user_id", "platform", name="uq_api_credential_user_platform"),
    )

    @property
    def platform_label(self):
        return API_PLATFORM_LABELS.get(self.platform, self.platform)


def get_or_create_api_credentials(user):
    """Returns {platform: ApiCredential} covering all of API_PLATFORMS for
    this user, creating whichever ones don't exist yet. Each platform's
    api_id/callback_id is generated once and then kept unchanged forever -
    tied to this user's account, not their password, so resetting or
    reactivating their login later never changes these."""
    existing = {c.platform: c for c in ApiCredential.query.filter_by(user_id=user.id).all()}
    created_any = False
...
class PaymentLink(db.Model):
    ...
    # Only set for payment links created through the external API (see
    # external_api.py) - i.e. from one of an admin's Android app,
    # website, or Telegram bot instead of directly inside this panel.
    # NULL for every link created the normal way, which is unaffected by
    # any of this.
    #  - source_platform: which of the three it came through.
    #  - callback_url: where webhooks.py posts a status update once
    #    Einpays confirms the payment (only if the caller provided one).
    #  - api_credential_id: whose API key was used, so the outbound
    #    callback above can be signed with that platform's callback_id.
    source_platform = db.Column(db.String(20), nullable=True)
    callback_url = db.Column(db.String(500), nullable=True)
    api_credential_id = db.Column(db.Integer, db.ForeignKey("api_credentials.id"), nullable=True)
    api_credential = db.relationship("ApiCredential")
